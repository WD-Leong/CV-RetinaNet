...
def build_model(
    ...
    for n_output in range(len(fpn_output)):
        ...
        for n_anchor in range(n_anchors):
            cen_layer_name = "cen_output_" + str(n_output+1)
            cen_layer_name += "_anchor_" + str(n_anchor+1)
            
            cen_output = layers.Conv2D(
                1, 3, 1, 
                padding="same", 
                use_bias=True, 
                name=cen_layer_name)(tmp_output)
            cen_anchors.append(cen_output)
        cen_heads.append(cen_anchors)
    ...
    for n_output in range(len(fpn_output)):
        ...
        reg_anchors = []
        for n_anchor in range(n_anchors):
            reg_layer_name = "arm_output_" + str(n_output+1)
    ...
    for n_level in range(len(fpn_output)):
        arm_anchors = []
        for n_anchor in range(n_anchors):
            arm_anchors.append(tf.concat([
                reg_heads[n_level][n_anchor], 
                cen_heads[n_level][n_anchor]], axis=3))
        arm_out.append(arm_anchors)
    
    # Transfer Connection Block. #
    rev_arm = arm_out[::-1]
    rev_shp = tf.shape(rev_arm[0][0])
    
    res_in = []
    for n_anchor in range(n_anchors):
        tmp_res_zeros = tf.zeros([
            rev_shp[0], rev_shp[1], rev_shp[2], 256], 
            name="res_zeros_init_"+str(n_anchor+1))
        res_in.append(tmp_res_zeros)
    ...
    for n_level in range(len(arm_out)):
        ...
        tcb_level = "tcb_" + str(n_level+1)
        tcb_name_1 = tcb_level + "_cnn_1"
        tcb_name_2 = tcb_level + "_cnn_2"
        tcb_name_3 = tcb_level + "_output"
        
        tcb_layer_1 = layers.Conv2D(
            256, 1, 1, "same", name=tcb_name_1)
        tcb_layer_2 = layers.Conv2D(
            256, 3, 1, "same", name=tcb_name_2)
        tcb_layer_3 = layers.Conv2D(
            256, 3, 1, "same", name=tcb_name_3)
        
        for n_anchor in range(n_anchors):
            tmp_tcb = tcb_layer_1(
                rev_arm[n_level][n_anchor])
            tmp_tcb = tcb_layer_2(tmp_tcb)
            tmp_tcb = tf.nn.relu(tmp_tcb)
            
            if n_level == 0:
                tmp_out = tmp_tcb + res_in[n_anchor]
            else:
                ups_name = "ups_" + str(n_level+1)
                ups_name += "_" + str(n_anchor+1)
                tmp_out = tmp_tcb + layers.UpSampling2D(
                    size=(2, 2), name=ups_name)(res_in[n_anchor])
            
            tmp_out = tf.nn.relu(tmp_out)
            tmp_out = tcb_layer_3(tmp_out)
            tmp_out = tf.nn.relu(tmp_out)
            tmp_res_con.append(tmp_out)
            ...
            reg_name = "reg_output_" + str(n_level+1)
            cls_name = "cls_output_" + str(n_level+1)
            
            reg_output = tf.nn.relu(layer_reg_output)
            reg_output = layers.Conv2D(
                4, 3, 1, padding="same", 
                name=reg_name+"_"+str(n_anchor+1))(reg_output)
            
            cls_output = tf.nn.relu(layer_cls_output)
            cls_output = layers.Conv2D(
                num_classes, 3, 1, padding="same",
                bias_initializer=b_focal, 
                name=cls_name+"_"+str(n_anchor+1))(cls_output)
            
            tmp_anc_out.append(tf.concat([
                reg_output, cls_output], axis=3))
        
        res_in = tmp_res_con
        obm_out.append(tmp_anc_out)
    obm_out = obm_out[::-1]
    ...
    for n_level in range(len(arm_out)):
        tmp_anchor_out = []
        for n_anchor in range(n_anchors):
            tmp_anchor_out.append([
                arm_out[n_level][n_anchor], 
                obm_out[n_level][n_anchor]])
        x_outputs.append(tmp_anchor_out)
    return tf.keras.Model(
        inputs=backbone.input, outputs=x_outputs)

# Define the FCOS model class. #
class RefineDet(tf.keras.Model):
    def __init__(
        self, n_classes, id_2_label, 
        aspect_ratios=None, anchor_scales=None, 
        anchor_sizes=None, backbone_model="resnet50", **kwargs):
        super(RefineDet, self).__init__(name="RefineDet", **kwargs)
    ...
    def train_loss(self, x_image, x_label):
        ...
        for n_scale in range(len(x_pred)):
            for n_anchor in range(self.n_anchors):
                gt_label = x_label[n_scale][n_anchor]
                tmp_obj  = tf.reduce_max(
                    gt_label[..., 5:], axis=-1)
                tmp_mask = tf.cast(tmp_obj > 0.0, tf.float32)
                
                arm_pred = x_pred[n_scale][n_anchor][0]
                obm_pred = x_pred[n_scale][n_anchor][1]
                
                cls_loss += self.focal_loss(
                    gt_label[..., 5:], obm_pred[0][..., 4:])
                
                cen_loss += self.focal_loss(
                    gt_label[..., 4], arm_pred[0][..., 4])
                
                reg_loss_arm += self.smooth_l1_loss(
                    gt_label[..., :4], tf.nn.sigmoid(
                        arm_pred[0][..., :4]), mask=tmp_mask)
                reg_loss_obm += self.smooth_l1_loss(
                    gt_label[..., :4], tf.nn.sigmoid(
                        obm_pred[0][..., :4]), mask=tmp_mask)
        return cen_loss, reg_loss_arm, cls_loss, reg_loss_obm
    ...
    def cpu_nms(self, dets, base_thr):
        ...
        while len(order) > 0:
            ...
            inter_area = w * h
            union_area = \
                areas[i] + areas[order[1:]] - inter_area + eps
            in_over_un = inter_area / union_area
            
            inds = np.where(in_over_un <= base_thr)[0]
            order = order[inds + 1]
    ...
    def image_detections(
        ...
        for n_level in range(len(tmp_predict)):
            ...
            for n_anchor in range(len(anchor_dim)):
                ...
                tmp_obm  = tmp_predict[n_level][n_anchor][1]
