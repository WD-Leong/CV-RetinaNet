...
def show_heatmap(
    ...
    for n_level in range(len(img_labels)):
        ...
        for n_anchor in range(len(anchor_dim)):
            ...
            tmp_anchor = anchor_dim[n_anchor]
            cls_output = tmp_output[n_anchor][..., 4]
            #cls_output = tmp_output[1][n_anchor][..., 4:]
            #max_probs  = tf.reduce_max(cls_output, axis=2)
            tmp_array[0::dwn_scale, 0::dwn_scale] = cls_output
            ...
            tmp_bboxes = prediction_to_corners(
                tmp_output[n_anchor][..., :4], tmp_anchor, stride)
            anc_output = tmp_output[
                n_anchor].reshape(-1, num_classes+5)
            tmp_scores = np.expand_dims(anc_output[:, 5:], axis=0)
            ...
            del tmp_bboxes, tmp_scores, anc_output
...
def train(
    ...
    tot_arm_reg_loss = 0.0
    tot_obm_reg_loss = 0.0
    tot_arm_cls_loss = 0.0
    tot_obm_cls_loss = 0.0
    ...
    for step in range(st_step, max_steps):
        ...
        for tmp_idx in batch_sample:
            ...
            arm_cls_losses += tmp_losses[0]
            arm_reg_losses += tmp_losses[1]
            obm_cls_losses += tmp_losses[2]
            obm_reg_losses += tmp_losses[3]
        ...
        tot_arm_reg_loss += arm_reg_losses / batch_size
        tot_arm_cls_loss += arm_cls_losses / batch_size
        tot_obm_reg_loss += obm_reg_losses / batch_size
        tot_obm_cls_loss += obm_cls_losses / batch_size
        ...
        if (step+1) % display_step == 0:
            ...
            avg_arm_reg_loss = tot_arm_reg_loss / display_step
            avg_arm_cls_loss = tot_arm_cls_loss / display_step
            avg_obm_reg_loss = tot_obm_reg_loss / display_step
            avg_obm_cls_loss = tot_obm_cls_loss / display_step
            
            avg_arm_reg_loss = avg_arm_reg_loss.numpy()
            avg_arm_cls_loss = avg_arm_cls_loss.numpy()
            avg_obm_reg_loss = avg_obm_reg_loss.numpy()
            avg_obm_cls_loss = avg_obm_cls_loss.numpy()
            
            training_loss.append((
                step+1, avg_loss.numpy(), 
                avg_arm_cls_loss, avg_arm_reg_loss, 
                avg_obm_cls_loss, avg_obm_reg_loss))
            ...
            print("Learning Rate:", 
                  str(optimizer.lr.numpy()))
            print("Average Objs:", str(avg_objs))
            print("Average Loss:", 
                  str(round(avg_loss.numpy(), 5)))
            print("Average ARM Reg Loss:", 
                  str(round(avg_arm_reg_loss, 5)))
            print("Average ARM Cls Loss:", 
                  str(round(avg_arm_cls_loss, 5)))
            print("Average OBM Reg Loss:", 
                  str(round(avg_obm_reg_loss, 5)))
            print("Average OBM Cls Loss:", 
                  str(round(avg_obm_cls_loss, 5)))
            ...
            if (step+1) % step_save == 0:
                ...
                df_columns = [
                    "step", "train_loss", "arm_cls_loss", 
                    "arm_reg_loss", "obm_cls_loss", "obm_reg_loss"]
...
train_loss = \
    model_path + "crowd_losses_refinedet_resnet50.csv"
ckpt_model = model_path + "crowd_refinedet_resnet50"
restore_flag = True
...
retinanet_model = refinedet_module.RefineDet(
